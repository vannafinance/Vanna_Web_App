...
const DropdownOption = memo(({ 
          ...
          {isChecked ? (
            <svg width="14" height="10" viewBox="0 0 14 10" fill="none">
              <path
                ...
                opacity="0.5"
              />
              <line x1="0" y1="0" x2="14" y2="10" stroke={isDark ? "#FFFFFF" : "#111111"} strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          ) : (
            <svg width="14" height="10" viewBox="0 0 14 10" fill="none">
              <path
  ...
  return (
    <div className={`w-full h-fit flex items-center gap-[4px] text-[14px] font-semibold ${
      ...
      <Checkbox checked={isChecked} onChange={onToggle} borderColor={isDark ? "#464545" : undefined} />
      {hasIcon && <Image src={iconPaths[item]} alt={item} width={20} height={20} />}
      {item}
    </div>
  );
          ...
          {props.dropDownType === "all-chains" && (
            <>
              {CHAIN_ICONS.map(([key, value], idx) => (
                <Image
                  key={key}
                  className={`${idx !== 0 ? "-ml-4" : ""} flex-shrink-0 rounded-full ${
      ...
      {isSmallDropdown && (
        <div className="w-fit h-fit rounded-[8px] flex items-center gap-[4px] min-w-0">
          ...
          {props.currentDropdownItem?.length === 0 ? (
            <div
              onClick={toggleDropdown}
              className={`cursor-pointer text-[12px] font-semibold ${isDark ? "text-white" : "text-black"} flex gap-[4px]`}
            >
