  ...
  return (
    ...
    >
      <div className="rounded-[8px]   font-medium text-[15.5px] cursor-pointer flex gap-2 justify-center items-center">
        <Image src={props.selectedOption.icon} width={20} height={20} alt={props.selectedOption.id}/> {props.selectedOption.name} 
      ...
      <AnimatePresence>
        {isHover && (
          <motion.div
            ...
            className="absolute  p-2 top-8 -left-4  shadow-lg rounded-[6px]"
          >
            {props.items.map((item, idx) => {
              return (
                <motion.div
                  ...
                  key={idx}
                  onClick={() => {props.setSelectedOption(item)
                  }}
                >
