...
// Memoized filter logic - extracted to prevent recreation
const applyFilters = (
  rows: TableProps["tableBody"]["rows"],
  searchLower: string,
  filtersState: FiltersState,
  supplyApyFilter: { percentage: number; greaterThan: boolean },
...
export const Table = memo((props: TableProps) => {
  ...
  const [supplyApyFilter, setSupplyApyFilter] = useState({
    percentage: 0,
    greaterThan: false,
  });
  const [currentPage, setCurrentPage] = useState(1);

  // Memoize column indices
  ...
  const showCustomizeDropdown = props.filters?.customizeDropdown;
  const hasCollateral = props.filters?.filters?.includes("Collateral") ?? false;
  const hasDeposit = props.filters?.filters?.includes("Deposit") ?? false;
  const hasAll = props.filters?.filters?.includes("All") ?? false;

  // Memoize customize options
