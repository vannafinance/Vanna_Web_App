...
export default function FarmDetailPage() {
  ...
  const farmData = useMemo(() => {
    if (!selectedRow || !selectedRow.cell || selectedRow.cell.length === 0) {
      return {
        title: id,
        titles: null,
        chain: "ETH",
    ...
    const firstCell = selectedRow.cell[0];
    const titles = firstCell.titles || null;
    const title = titles 
      ? titles.join(" / ")
      : firstCell.title || id;
    const chain = firstCell.chain || "ETH";
  ...
  const isMultiAsset = tabType === "multi" && farmData.titles && farmData.titles.length > 1;
  ...
  return (
    <main className="flex flex-col gap-[40px] pt-[40px] px-[40px] pb-[80px]">
          ...
          <div className="w-full h-fit flex gap-[16px] items-center">
            ...
            <div className="flex gap-[16px]">
              ...
              <div className="w-fit h-fit flex gap-[8px] items-center">
                ...
                <div className="w-fit h-fit flex gap-[8px] items-center">
                  {farmData.tags.slice(0, 2).map((tag, index) => (
                    <span
                      key={index}
      ...
      <section className="w-full h-fit flex gap-[20px] ">
        <div className="w-full h-fit flex flex-col gap-[24px] rounded-[20px] border-[1px] p-[24px] bg-[#F7F7F7]">
